...
void capturarDato(struct CampoDeInformacion *data){
 ...
 int main(){  
    ...
    centerTitle("- Calculadora de número de dígitos -");
	...
	int digitoCapturado = atoi(digito.valor);
	
	int nDigitos=0;
	do{
		if(digitoCapturado > 0){
			nDigitos++;
			digitoCapturado = digitoCapturado/10; 
		}
		
	}while(digitoCapturado > 0);
	
	struct dataCord resultadoLine = { 1, 5 }; gotoxy(&resultadoLine);
	
	printf("El numero de digitos en el valor captuado es igual a %d",nDigitos);
